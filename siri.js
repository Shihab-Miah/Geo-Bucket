...
async function geoManualRegister(email, phone) {
   try {
      const cleanDigits = (phone || "").toString().replace(/\D/g, '');
      if (!cleanDigits || cleanDigits.length < 7 || cleanDigits.length > 15) {
         return { success: false, error: "Valid phone number (7-15 digits) is required." };
      }
      ...
      const r = await fetch(GEO_APP_SCRIPT_URL, {
         method: 'POST',
         headers: { 'Content-Type': 'text/plain;charset=utf-8' },
         body: JSON.stringify({ action: "register", googleId: fakeGoogleId, email, phone, location })
      });
      const result = await r.json();
      if (result.success) {
...
async function geoLoginGoogle(phone) {
   // SECURITY CHECK: Phone number is strictly mandatory
   const cleanDigits = (phone || "").toString().replace(/\D/g, '');
   if (!cleanDigits || cleanDigits.length < 7 || cleanDigits.length > 15) {
       return { success: false, error: "Security validation error: A valid phone number (7-15 digits) is strictly required to register." };
   }
...
async function showLicensingOverlay(status, paymentUrl, closable = false) {
   ...
   const countryList = [
      ...
      {"name":"Central African Rep.","code":"CF","dial_code":"+236"},
      ...
      {"name":"Trinidad & Tobago","code":"TT","dial_code":"+1"},
   ...
   googleBtn.onclick = async () => {
      ...
      if (!phoneInput) {
          loginMsgBox.textContent = "Security validation error: Phone input field required.";
      ...
      if (!phoneVal || phoneVal.length < 7 || phoneVal.length > 15) {
